...
extension Publishers {
    ...
    final class Timer<S: Scheduler>: ConnectablePublisher {
        ...
        private lazy var routingSubscription: RoutingSubscription = .init(parent: self)
            ...
            Downstream.Failure == Never
        {
            ...
            func timerFired() {
                lock.lock()
                guard let ds = downstream, let parent else {
                    lock.unlock()
                    return
                }
